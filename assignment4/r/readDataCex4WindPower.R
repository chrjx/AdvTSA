...
## Start
rm(list=ls())
setwd("C:\\Users\\tamas\\OneDrive - Danmarks Tekniske Universitet\\Advanced TS\\AdvTSA\\assignment4\\r")
...
plotAroundGaps <- function(D, cln, width, ylimAll = TRUE, i = NA) {
  ...
  # Fix: properly check if 'i' is NA
  if (all(is.na(i))) {
    i <- cumsum(grpLen)[grpNA]
  }
  
  if (plotIt) {
    for (ii in seq_along(i)) {
      # Prevent indexing outside range
      start_idx <- max(1, i[ii] - width)
      end_idx <- min(nrow(D), i[ii] + width)
      
      X <- D[start_idx:end_idx, ]
      ylim <- if (ylimAll) range(D[, cln], na.rm = TRUE) else range(X[, cln], na.rm = TRUE)
...
## Write the data file
write.table(X, "C:\\Users\\tamas\\OneDrive - Danmarks Tekniske Universitet\\Advanced TS\\AdvTSA\\assignment4\\r\\cex4WindDataNoInterpolation.csv", sep=",", row.names=FALSE)
